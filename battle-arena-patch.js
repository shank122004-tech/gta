...
(function (global) {
  ...
  const POLL_ACTIVE_GAME = 200;             // ms — OPTIMIZED: reduced to 200ms for fast question delivery (Firebase cost optimized batch writes)
  const POLL_WAITING_ROOM = 1500;           // ms — waiting room optimized polling
  ...
  const battleState = {
    ...
    lastRenderTime: 0,
    renderDebounceMs: 50,        // OPTIMIZED: reduced from 100ms to 50ms for faster updates
    lastQuestionIndex: -1,
  ...
  window.BA = {
    ...
    _openBattle(battleId, battleData) {
      ...
      // Start polling with ADAPTIVE intervals based on battle status
      if (this._pollGameInterval) clearInterval(this._pollGameInterval);
      const pollInterval = battleData.status === 'waiting' ? POLL_WAITING_ROOM : POLL_ACTIVE_GAME;
      this._pollGameInterval = setInterval(() => this._pollGameBattle(battleId), pollInterval);
    ...
    async _pollGameBattle(battleId) {
      ...
      const now = Date.now();
      if (now - this._lastPollTime < 50) return;  // REDUCED from 100ms to 50ms
      ...
      try {
        ...
        if (data.status === 'generating') {
          ...
          if (!data.questions || data.questions.length === 0) {
            setTimeout(() => this._pollGameBattle(battleId), 150);  // REDUCED from 200ms
            return;
          }
          ...
          // CRITICAL FIX: Only skip render if progress matches AND user is answering the SAME question
          const progressKey = myUid + '|' + userProgress;
          const lastProgressKey = this._lastProgressKey;
          
          // If progress changed, force immediate re-render
          if (lastProgressKey !== progressKey) {
            this._lastProgressKey = progressKey;
            this._lastRenderedQi = -1;  // Force re-render
            this._lastRenderHash = null;
            this._stopQuestionTimer();
            battleState.forceRender();  // Force immediate render
          }
    ...
    _handleCountdown(data, battleId) {
      ...
      const tick = () => {
        ...
        if (remaining > 0) {
          ...
          setTimeout(tick, 50);  // OPTIMIZED: reduced from 100ms to 50ms for smoother countdown
        } else {
          tickRunning = false;
          if (numEl) {
          ...
          setTimeout(() => {
            ...
            this._pollGameBattle(battleId);
          }, 500);  // OPTIMIZED: reduced from 800ms
        }
      };

      setTimeout(tick, 50);  // Start immediately with 50ms interval
    },
    ...
    _renderActiveQuiz(battle) {
      ...
      // CRITICAL FIX: Track question index strictly to prevent backwards jumps
      if (this._lastRenderedQi > qi) {
        // Question index went backwards — force full re-render
        this._lastRenderHash = null;
        this._stopQuestionTimer();
      }
      
      // CRITICAL: If hash is same and question timer exists, skip re-render
      if (!hashChanged && this._lastRenderedQi === qi && document.getElementById('ba-qtimer-fill')) {
        ...
        // OPTIMIZATION: Update existing XP board if it changed (always check)
    ...
    async _submitAnswer(battleId, qi, chosenIdx) {
      ...
      if (this._activeBattleId) {
        ...
        setTimeout(() => {
          this._lastRenderHash = null;  // Force re-render
          battleState.lastQuestionIndex = -1;  // Force re-render of next question
          battleState.forceRender();
          this._pollGameBattle(bid);
        }, 100);  // OPTIMIZED: reduced from 150ms for faster next question
      }
    },
